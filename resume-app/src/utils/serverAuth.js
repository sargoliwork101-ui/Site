 ...
 * پل فرانت به بک‌اند PHP. همه آدرس‌ها نسبی‌ان (api/...) تا سایت در ساب‌فولدر
 * هم بدون تنظیمات کار کنه. اگه api/ روی هاست نباشه، همه توابع با خطای
 * 'no_backend' برمی‌گردن و اپ وارد «حالت محلی» می‌شه (نه کرش).
 * گروه‌ها: وضعیت/نصب (fetchServerStatus, serverSetup, ...) — ورود/خروج —
 * فراموشی رمز (OTP) — تغییر ایمیل — صندوق SMTP — بک‌آپ‌های هاست —
 * آپلود فایل تماس — ذخیره/خواندن محتوا — ارسال پیام تماس.
 * ⚠️ برای اکشن جدید سمت سرور: اول auth.php رو تغییر بده، بعد یه تابع مشابه
 * همین‌جا اضافه کن. تایماوت همه درخواست‌ها ۲۰ ثانیه (آپلود ۶۰ ثانیه).
 */
