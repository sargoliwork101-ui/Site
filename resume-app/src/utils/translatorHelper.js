/**
 * ═══════════════════════════════════════════════════════════════════
 * translatorHelper.js — دیکشنری خودترجمه فارسی→انگلیسی (پیشنهاد خودکار)
 * ═══════════════════════════════════════════════════════════════════
 * این فایل چیه؟ وقتی ادمین فیلد فارسی رو پر می‌کنه و فیلد انگلیسی خالیه،
 * این موتور یه پیشنهاد خودکار می‌سازه تا ادمین از صفر تایپ نکنه.
 * کِی بازش کن؟ اضافه کردن اصطلاح تخصصی جدید: کافیه یه خط به دیکشنری
 *   technicalDictFaToEn اضافه کنی: 'عبارت فارسی': 'English phrase',
 * ⚠️ این «ترجمه ماشینی» نیست — فقط جایگزینی عبارتی + slug تمیز. متن نهایی
 *   انگلیسی همیشه با ادمینه (فیلد دستی زیر فیلد فارسی).
 */

/** دیکشنری عبارت‌های تخصصی. کلید باید دقیقاً همون عبارتی باشه که ادمین تایپ می‌کنه. */
...
/**
 * پیشنهاد انگلیسی برای یه متن فارسی.
 * ترتیب کار: (۱) اگه کل متن دقیقاً تو دیکشنری بود همون (۲) وگرنه جایگزینی
 * تکه‌تکه عبارات شناخته‌شده (۳) اگه هنوز حروف فارسی موند، به‌جاشون عبارت
 * خنثی می‌ذاره تا خروجی همیشه قابل نمایش باشه.
 * @param {string} faText - متن فارسی ورودی
 * @returns {string} پیشنهاد انگلیسی (ممکنه ناقص باشه — ادمین بازبینی می‌کنه)
 */
