...
export const DataProvider = ({ children }) => {
    ...
    try {
      ...
      if (saved) {
        ...
        const mergedBoards = (cleanParsed.boards && cleanParsed.boards.length >= initialData.boards.length)
          ? cleanParsed.boards
          : initialData.boards;
        // Backfill academic-paper fields (authors/venue/year/type/doi) for
        // articles saved before those fields existed — never overwrite user edits.
        const backfillPaperFields = (list) =>
          (list || []).map((a) => {
            const def = initialData.articles.find((d) => d.id === a.id) || {};
            return { ...def, ...a };
          });
        const mergedArticles = backfillPaperFields(
          cleanParsed.articles && cleanParsed.articles.length >= initialData.articles.length
  ...
  // Site page routes — same page structure as the original site
  // (home / about / works / papers / contact) with shareable hash URLs
  const SITE_PAGES = ['about', 'works', 'papers', 'contact'];
  const [route, setRoute] = useState(() => {
    if (typeof window !== 'undefined') {
      const m = window.location.hash.match(/^#\/([a-z]+)/i);
      if (m && ['about', 'works', 'papers', 'contact'].includes(m[1])) return m[1];
    }
    return 'home';
  });

  // Dedicated route navigation helper (hash-based, shareable URLs)
  const navigate = (page = 'home') => {
    if (page === 'home') {
  ...
  useEffect(() => {
    const handleHash = () => {
      ...
      const page =
        pageMatch && SITE_PAGES.includes(pageMatch[1]) ? pageMatch[1] : 'home';
      setRoute(page);
      // Page changes always start from the top (like a real multi-page site)
