...
export const TaxonomyManagerModal = ({
  isOpen,
  onClose,
  initialTab = 'boardCategories' // 'boardCategories' | 'articleCategories' | 'boardStatuses' | 'edaTools'
}) => {
  const {
    data,
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/85 backdrop-blur-md p-3 sm:p-6 animate-fadeIn">
      ...
      >
        ...
        {/* Tab Navigation */}
        <div className="flex items-center gap-1.5 p-3 bg-slate-950/60 border-b border-slate-800/80 overflow-x-auto">
          {tabConfigs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              ...
              >
                ...
                <span className={`px-1.5 py-0.2 rounded text-[10px] font-mono ${active ? 'bg-slate-950 text-cyan-300' : 'bg-slate-800 text-slate-400'}`}>
