/**
 * ═══════════════════════════════════════════════════════════════════
 * RichTextEditorModal.jsx — ادیتور متنی شبیه ورد (فقط ادمین)
 * ═══════════════════════════════════════════════════════════════════
 * چی توشه؟ نوار ابزار ورد (فونت/رنگ/تراز/لیست/جدول‌ساز/لینک/تصویر/کادر نکته/
 * نمادهای مهندسی Ω⚡)، دو حالت دیداری و سورس HTML، ایمپورت DOCX/TXT/MD/HTML،
 * پیست تمیزشده از ورد، جستجو/جایگزینی، خروجی DOC/HTML/TXT، و Ctrl+S واقعی.
 * ورودی onSave/onClose/initialValue؛ فقط داخل پنل استفاده می‌شه.
 * ⚠️ لینک و تصویر ورودی sanitize می‌شن — sanitize رو دور نزن. خروجی ادیتور
 * همیشه HTMLـه و موقع نمایش با sanitizeRichHtml تمیز می‌شه (دفاع دو‌لایه).
 */
...
export const RichTextEditorModal = ({
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const imgHtml = `<img src="${base64}" alt="${safeAlt}" loading="lazy" decoding="async" style="max-width: 100%; border-radius: 12px; margin: 16px 0; box-shadow: 0 4px 20px rgba(0,0,0,0.3);" />`;
      if (editorRef.current) {
        editorRef.current.focus();
