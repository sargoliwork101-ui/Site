...
export const BoardsSection = () => {
  ...
  // Dynamic board categories from taxonomies with fallback
  const categories = useMemo(() => {
    const rawCategories = data?.taxonomies?.boardCategories || [];
    const list = [
  ...
  return (
    <section id="boards" className="py-20 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Filter Controls & Search Bar & Sort Dropdown */}
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {categories.map((cat) => {
              ...
              const count = cat.id === 'all' 
                ? (data.boards || []).length 
                : (data.boards || []).filter(b => b.category === cat.id || b.categoryFa === cat.labelFa).length;
              ...
              return (
                <button
                  ...
                  title={isFa ? `فیلتر بر اساس دسته: ${cat.labelFa}` : `Filter by category: ${cat.labelEn}`}
                ...
                >
                  ...
                  <span
                    className={`text-[10px] px-1.5 py-0.2 rounded-full font-mono ${
                      active ? 'bg-slate-950/20 text-slate-950 font-bold' : 'bg-slate-800 text-slate-400'
                    }`}
                  >
                    {formatNum(count, isFa)}
                  </span>
                </button>
              );
            })}
