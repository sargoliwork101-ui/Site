...
export const AdminPanel = () => {
  ...
  const handlePersonalSubmit = (e) => {
    ...
    // Home page content (hero headline / subtitle / intro) — like the site
    if (
      formData.has('headlineFa') || formData.has('headlineEn') ||
      formData.has('subtitleFa') || formData.has('subtitleEn') ||
      formData.has('introFa') || formData.has('introEn')
    ) {
      updateHome({
        headlineFa: formData.get('headlineFa') || '',
        headlineEn: formData.get('headlineEn') || '',
        subtitleFa: formData.get('subtitleFa') || '',
        subtitleEn: formData.get('subtitleEn') || '',
        introFa: formData.get('introFa') || '',
  ...
  const handleArticleSave = (e) => {
    ...
    // ---- Academic paper fields (like the site: authors / venue / year / type / DOI) ----
    const authorsFa = formData.get('authorsFa') || '';
    const authorsEn = formData.get('authorsEn') || authorsFa;
    const venueFa = formData.get('venueFa') || '';
    const venueEn = formData.get('venueEn') || venueFa;
    const year = (formData.get('year') || '').trim();
    const type = formData.get('type') || 'other';
    const doi = (formData.get('doi') || '').trim();
          ...
          <main className="flex-1 p-4 sm:p-6 overflow-y-auto bg-slate-900/50">
            ...
            {activeTab === 'personal' && (
              <form onSubmit={handlePersonalSubmit} className="space-y-6 max-w-4xl">
                ...
                {/* 0. Home Page Content — like the site's «ویرایش محتوا» */}
                <div className="p-4 rounded-2xl bg-slate-900/60 border border-cyan-500/30 space-y-3">
                  <label className="text-xs font-bold text-cyan-300 flex items-center gap-2">
                    ...
                    <span>ویرایش محتوا — صفحه اصلی (Hero + معرفی کوتاه)</span>
                  </label>
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    این متون دقیقاً در هدر و کارت معرفی صفحه اصلی سایت (تم روشن) نمایش داده می‌شوند.
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] text-slate-300 mb-1">تیتر اصلی هدر (فارسی):</label>
                      <input
                        name="headlineFa"
                        defaultValue={data.home?.headlineFa || ''}
                        placeholder="سلام! من «نام شما» هستم"
                        className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] text-cyan-400 mb-1">Hero Headline (English):</label>
                      <input
                        name="headlineEn"
                        defaultValue={data.home?.headlineEn || ''}
                    ...
                    <div>
                      <label className="block text-[11px] text-slate-300 mb-1">زیرتیتر معرفی (فارسی):</label>
                      <input
                        name="subtitleFa"
                        defaultValue={data.home?.subtitleFa || ''}
                        placeholder="مهندس الکترونیک — طراحی PCB و ..."
                        className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] text-cyan-400 mb-1">Hero Subtitle (English):</label>
                      <input
                        name="subtitleEn"
                        defaultValue={data.home?.subtitleEn || ''}
                        placeholder="Electronics Engineer — PCB Design ..."
                        className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white font-mono"
                        dir="ltr"
                      />
                    </div>
                    <div className="sm:col-span-2">
                      <label className="block text-[11px] text-slate-300 mb-1">متن معرفی کوتاه (فارسی):</label>
                      <textarea
                        name="introFa"
                        defaultValue={data.home?.introFa || ''}
                        rows="2"
                        className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white leading-relaxed"
                      />
                    </div>
                    <div className="sm:col-span-2">
                      <label className="block text-[11px] text-cyan-400 mb-1">Short Introduction (English):</label>
                      <textarea
                        name="introEn"
                        defaultValue={data.home?.introEn || ''}
                        rows="2"
                        className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white leading-relaxed font-mono"
                        dir="ltr"
                      />
                    </div>
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-950/70 border border-slate-800 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                    ...
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1">ORCID</label>
                      <input
                        name="orcid"
                        defaultValue={info.orcid || ''}
                    ...
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1">Google Scholar</label>
                      <input
                        name="scholar"
                        defaultValue={info.scholar || ''}
              ...
              <div className="space-y-6 max-w-5xl">
                {articleForm ? (
                  <form onSubmit={handleArticleSave} className="space-y-4 p-6 rounded-2xl bg-slate-950/80 border border-slate-800">
                    ...
                    {/* Academic Paper Metadata (like the site: authors / venue / year / type / DOI) */}
                    <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-3">
                      <label className="text-xs font-bold text-cyan-400 flex items-center gap-2">
                        📚 مشخصات علمی مقاله (مقالات علمی سایت)
                      </label>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[11px] text-slate-300 mb-1">نویسندگان (فارسی):</label>
                          <input
                            name="authorsFa"
                            defaultValue={articleForm.authorsFa || ''}
                            placeholder="مثال: آرش طاهری، مهسا کریمی"
                            className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white"
                          />
                        </div>
                        <div>
                          <label className="block text-[11px] text-cyan-400 mb-1">Authors (English):</label>
                          <input
                            name="authorsEn"
                            defaultValue={articleForm.authorsEn || ''}
                            placeholder="Arash Taheri, Mahsa Karimi"
                            className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white font-mono"
                          />
                        </div>
                        <div>
                          <label className="block text-[11px] text-slate-300 mb-1">مجله / کنفرانس / دانشگاه (فارسی):</label>
                          <input
                            name="venueFa"
                            defaultValue={articleForm.venueFa || ''}
                            placeholder="مثال: ژورنال بین‌المللی مهندسی الکترونیک"
                            className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white"
                          />
                        </div>
                        <div>
                          <label className="block text-[11px] text-cyan-400 mb-1">Venue / Journal (English):</label>
                          <input
                            name="venueEn"
                            defaultValue={articleForm.venueEn || ''}
                            placeholder="Int. Journal of Electronic Engineering"
                            className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white font-mono"
                          />
                        </div>
                        <div>
                          <label className="block text-[11px] text-slate-300 mb-1">سال انتشار (شمسی/میلادی):</label>
                          <input
                            name="year"
                            defaultValue={articleForm.year || ''}
                            placeholder="مثال: 1403 یا 2024"
                            className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white font-mono"
                          />
                        </div>
                        <div>
                          <label className="block text-[11px] text-slate-300 mb-1">نوع اثر:</label>
                          <select
                            name="type"
                            defaultValue={articleForm.type || 'journal'}
                            className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white"
                          >
                            <option value="journal">مقاله ژورنالی (Journal)</option>
                            <option value="conference">مقاله کنفرانسی (Conference)</option>
                            <option value="book_chapter">فصل کتاب (Book Chapter)</option>
                            <option value="book">کتاب (Book)</option>
                            <option value="thesis">پایان‌نامه (Thesis)</option>
                            <option value="report">گزارش فنی (Technical Report)</option>
                            <option value="other">سایر (Other)</option>
                          </select>
                        </div>
                        <div className="sm:col-span-2">
                          <label className="block text-[11px] text-slate-300 mb-1">شناسه DOI (اختیاری):</label>
                          <input
                            name="doi"
                            defaultValue={articleForm.doi || ''}
                            placeholder="10.1109/... یا https://doi.org/..."
                            className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white font-mono"
                            dir="ltr"
                          />
                        </div>
