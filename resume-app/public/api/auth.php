...
/**
 * ── فارسی: قلب امنیت سرور ──
 * همه احراز هویت این‌جاست: نصب اول، لاگین (bcrypt/Argon2)، فراموشی رمز با
 * OTP ایمیلی، تغییر رمز/ایمیل، صندوق SMTP، و بک‌آپ‌های روی هاست. فرانت فقط
 * از طریق serverAuth.js باهاش حرف می‌زنه. جواب‌ها همیشه JSONـه.
 * ⚠️ اکشن جدید: یه case جدید + چک سشن ادمین اگه حساسه + تابع هم‌نام در
 * serverAuth.js. هرگز هش/توکن/رمز رو echo یا error_log نکن. ریت‌لیمیت‌ها
 * (RL_*) در config.phpـه — دست‌کاری‌شون امنیت رو شل می‌کنه.
 */
