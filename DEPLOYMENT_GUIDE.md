...
> ⚡ **عجله داری؟** کافی است فایل **`site-upload.zip`** (ریشه ریپو) را داخل `public_html` اکسترکت کنی و با `Ctrl+Shift+R` رفرش سنگین بزنی — قدم‌به‌قدمش در **[QUICK_DEPLOY_FA.md](QUICK_DEPLOY_FA.md)**. ادامه این فایل، جزئیات دستی و پیشرفته (بیلد محلی، VPS، ساب‌دامین وبلاگ) را پوشش می‌دهد.
...
### گام ۲: فایل روتینگ و کش سرور (`.htaccess` — آماده داخل پروژه)
فایل **`.htaccess`** از قبل در `resume-app/public/` موجود است و با `npm run build` خودکار داخل `dist` کپی می‌شود — **لازم نیست دستی بسازید**؛ فقط مطمئن شوید موقع آپلود منتقل شده باشد (فایل‌های نقطه‌دار گاهی در File Manager مخفی‌اند). این فایل باعث می‌شود در صورت رفرش صفحات خطای ۴۰۴ رخ ندهد، سرعت با Gzip و کش بالا برود، و دسترسی وب به پوشه محرمانه `api/data/` بسته شود. محتوای مرجع (نسخه کامل در خود فایل):
...
3. **ایجاد کانفیگ Nginx در مسیر `/etc/nginx/sites-available/my-resume`:**
   ...
   server {
       ...
       # PHP backend (auth/mail/backup APIs) — requires php-fpm on the server:
       #   sudo apt install php-fpm php-mbstring -y
       location ~ \.php$ {
           include snippets/fastcgi-php.conf;
           fastcgi_pass unix:/run/php/php8.2-fpm.sock; # ← نسخه PHP خودت را بگذار
       }
   }
   ```

> ⚠️ بدون بلوک `\.php$` بالا، فایل‌های `api/*.php` اجرا نمی‌شوند و سایت به «حالت محلی» می‌افتد (لاگین سروری، ایمیل و بک‌آپ هاست کار نمی‌کند). بعد از تغییر کانفیگ حتماً `sudo nginx -t && sudo systemctl reload nginx` بزنید.
...
6. **فرستنده ایمیل:** اگر صندوق SMTP در پنل پیکربندی شده باشد، همه اعلان‌ها با همان صندوق ارسال می‌شوند؛ در غیر این صورت از `mail()` خود هاست استفاده می‌شود (بدون واسطه خارجی). برای deliverability بهتر، در `api/config.php` ثابت `MAIL_FROM` را یک آدرس روی دامنه خودتان بگذارید (مثل `noreply@yourdomain.ir`).
7. **📬 صندوق ارسال اعلان‌ها (SMTP):** در پنل ← تب «امنیت و رمز عبور» کارت «صندوق ارسال اعلان‌ها» را باز کنید: هاست، پورت، نوع رمزنگاری، نام کاربری و رمز ایمیل را وارد/ویرایش کنید، ذخیره کنید و با دکمه «ارسال ایمیل تست» اتصال را بررسی کنید (ایمیل تست فقط به ایمیل بازیابی شما ارسال می‌شود). رمز فقط روی سرور (فایل 0600) ذخیره می‌شود و دکمه 👁 آن را فقط در همین نشست ادمین نشان می‌دهد.
...
## 🌐 ۷. راهنمای اتصال وبلاگ به ساب‌دامین اختصاصی (`blog.yourdomain.com`)
...
## 💾 ۸. پشتیبان‌گیری و بازگردانی آسان (Backup & Migration)

مرکز همه‌چیز: پنل ← تب **«پشتیبان‌گیری و دیتابیس»** (دکمه «پشتیبان‌گیری» بالای پنل فقط میان‌بر همین‌جاست).

- **فایل کامل JSON** (دکمه «دانلود فایل پشتیبان کامل»): کل سایت — محتوا، کاربران، امنیت و سؤالات بازیابی. برای انتقال به هاست جدید همین فایل را با «بارگذاری فایل پشتیبان» ایمپورت کنید (فایل‌های قدیمی هم خوانده می‌شوند).
- **بک‌آپ خودکار روی هاست:** اگر تا پایان زمان تنظیمی (۱ تا ۳۰ روز، پیش‌فرض ۷) تغییری بدهید و بعد ساکت بمانید، فقط **یک** بک‌آپ در `api/data/backups/` ذخیره می‌شود. فقط N تای آخر نگه داشته می‌شود (پیش‌فرض ۲، قابل تنظیم ۱ تا ۱۰).
- **تک‌تک نسخه‌ها** (هم هاست هم دستی) دکمه دانلود جدا دارند؛ بازیابی هر نسخه با دیالوگ تایید و گرفتن بک‌آپ احتیاطی از وضعیت فعلی انجام می‌شود.
- **کجا هستند؟** هاست: `api/data/backups/` · دستی: حافظه همان مرورگر (با پاک‌شدن دیتای مرورگر می‌پرند!) · فایل: پوشه Downloads شما. همیشه یک فایل تازه جای امن داشته باشید.
